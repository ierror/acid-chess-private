...
class RandomRotation(T.RandomRotation):
    def forward(
        ...
        degrees = choice([45, 90])
        image = F.rotate(image, degrees)
        if target is not None:
            if "boxes" in target:
                target["boxes"][0] = F.rotate(target["boxes"][0], degrees)
            if "masks" in target:
                target["masks"][0] = F.rotate(target["masks"][0], degrees)
            if "keypoints" in target:
                target["keypoints"][0] = F.rotate(target["keypoints"][0], degrees)
        return image, target
